...
```{r sequencing-summary}
ssl_bismark_output %>%
  group_by(locus) %>%
  summarise(mean_reads = mean(`Total Reads`)) %>%
  arrange(mean_reads)
...
```{r}
meta %>%
  mutate(CollectionDate = paste0(CollectionYear,"-",CollectionMonth,"-",CollectionDay)) %>%
  filter(ExtractionPlate == "DNA0147") %>%
  select(AlternateID, CollectionDate, Location1, AgeYRS, Sex) %>%
  arrange(AgeYRS) %>%
  write_csv("outputs/DNA0147_samples_for_table.csv")
