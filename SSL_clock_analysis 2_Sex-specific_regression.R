...
#Data in vectors - rank of Pearson r
Age=df$Age
Pop=df$Location1
Sex=df$Sex
...
modelp2=lm(formula=predictions~Age*Sex)
modelp3=lm(formula=predictions~Age+Sex)
AIC(modelp,modelp2, modelp3)

#Plotting for multiple linear regression
pdata<-data.frame(Age,predictions)
plot1<-ggplot(pdata,aes(y=predictions,x=Age, color=Sex))+geom_smooth(method="lm", fill = NA)+
  geom_point(shape=1, size=4)+xlab('Known age')+ylab('Predicted age')+
  #stat_poly_eq(formula = y ~ x, aes(label = after_stat(eq.label))) +
  xlim(4, 26)+ylim(4, 26) +
  scale_color_manual(values = c("F" = "darkred", "M" = "blue")) +
  theme(axis.title = element_text(size = 12)) +
  theme(axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10))+
   theme(legend.position.inside = c(0.8, 0.15))

ggsave("sex_all.pdf", plot1)

##Subset the data to maximum age of males, repeat analysis
...
#Plotting for multiple linear regression
pdata<-data.frame(Age,predictions)
plot2<-ggplot(pdata,aes(y=predictions,x=Age, color=Sex))+geom_smooth(method="lm", fill = NA)+
  geom_point(shape=1, size=4)+xlab('Known age')+ylab('Predicted age')+
  #stat_poly_eq(formula = y ~ x, aes(label = after_stat(eq.label))) +
  xlim(4, 18)+ylim(4, 18) +
 scale_color_manual(values = c("F" = "darkred", "M" = "blue")) +
